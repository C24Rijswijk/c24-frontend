  ...
  try {
    ...
    if (req.files) {
      ...
      if (req.files[req.body.file]) {
        const originalFileName = req.files[req.body.file].name;
        const extension = originalFileName.split(".").pop();
        req.files[req.body.file].name = new Date().getTime() + "." + extension;
    ...
    for (const [key, value] of Object.entries(req.body)) {
      if (key !== "table") {
        if (value.values === "flush") {
          ...
          query +=
            " id in (select user_id from company_details where hourly_rate <= ?  )";
